class ShoppingList
  ...
  def get(*arg)
    p "Getting now!!"
    @internal_hash[arg[:id]][arg[:perk]][arg[:info]]
  end
  
  def method_missing(name, *args)
    method_name = "get"
    perk_and_info = name[4..-1]
    hash = {}
    perk = perk_and_info.split("").take(perk_and_info.rindex("_")).join("")
    info = perk_and_info.split("").drop(perk_and_info.rindex("_")).join("")
    hash[:perk] = perk
    hash[:info] = info
    hash[:id] = args.first
    get(hash)
